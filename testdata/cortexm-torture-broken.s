@ cortexm-torture-broken.s — gêmeo de cortexm-torture.s com UMA checagem deliberadamente errada
@ (SDIV: espera 15 em vez de 14) — prova que o harness de teste realmente detecta falha (exit != 0).
...
div_check:
    ...
    movs r1, #7
    sdiv r2, r0, r1
    cmp r2, #15            @ ERRADO DE PROPÓSITO (correto: 14) — prova o harness de teste
    beq udiv_check
    fail 12
udiv_check:
    movs r0, #100
